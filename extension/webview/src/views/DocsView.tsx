...
/** Fixed display order for categories — maps category key to display label. */
...
export function DocsView({ vm }: { vm: ViewModel }) {
  ...
  // Group docs by category, falling back to '__other__' for uncategorized.
  ...
  // Sort within each group: by order (ascending), then title alphabetically.
  for (const group of groups.values()) {
    group.sort((a, b) => {
      const oa = a.order ?? 999;
      const ob = b.order ?? 999;
      if (oa !== ob) return oa - ob;
      return (a.title ?? labelFromPath(a.path)).localeCompare(b.title ?? labelFromPath(b.path));
    });
  }
